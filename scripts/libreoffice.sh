...
# -------
    ...
    start() {
        ...
        if [ $OFFICE_PORT -ne 0 ]; then
            ...
            /bin/su -s /bin/bash $USER -c "/opt/libreoffice5.3/program/soffice.bin \"--accept=socket,host=localhost,port=8100;urp;StarOffice.ServiceManager\" \"-env:UserInstallation=file:///opt/alfresco/alf_data/oouser\" --nologo --headless --nofirststartwizard --norestore --nodefault &" >/dev/null
