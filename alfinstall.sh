...
export TOMCAT_DOWNLOAD=ftp://ftp.sunet.se/pub/www/servers/apache/dist/tomcat/tomcat-7/v7.0.53/bin/apache-tomcat-7.0.53.tar.gz
...
if [ "$installtomcat" = "y" ]; then
  ...
  sed -i "s/@@LOCALESUPPORT@@/$LOCALESUPPORT/g" $BASE_DOWNLOAD/tomcat/alfresco.conf
  # Create /shared
...
# Always add the addons dir and scripts
  ...
  if [ ! -f "$ALF_HOME/scripts/libreoffice.sh" ]; then
    ...
    sed -i "s/@@LOCALESUPPORT@@/$LOCALESUPPORT/g" $BASE_DOWNLOAD/scripts/libreoffice.sh
  fi
