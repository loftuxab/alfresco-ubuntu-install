...
export TOMCAT_DOWNLOAD=ftp://ftp.sunet.se/pub/www/servers/apache/dist/tomcat/tomcat-7/v7.0.47/bin/apache-tomcat-7.0.47.tar.gz
...
export LIBREOFFICE=http://ftp.sunet.se/pub/Office/tdf/libreoffice/stable/4.1.3/deb/x86_64/LibreOffice_4.1.3_Linux_x86-64_deb.tar.gz
...
EOF
  ...
  if [ "$installtomcat" = "y" ]; then
    ...
    sudo sed -i 's@<referer></referer>@<referer>http://'"$HOSTNAME"'/.*|https://'"$HOSTNAME"'/.*</referer>@g' $SHARE_SECURITY
    sudo sed -i 's@<origin></origin>@<origin>http://'"$HOSTNAME"'|https://'"$HOSTNAME"'</origin>@g' $SHARE_SECURITY
  fi
  echo
