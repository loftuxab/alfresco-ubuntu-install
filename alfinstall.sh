...
export TOMCAT_DOWNLOAD=ftp://ftp.sunet.se/pub/www/servers/apache/dist/tomcat/tomcat-7/v7.0.42/bin/apache-tomcat-7.0.42.tar.gz
...
if [ "$installsolr" = "y" ]; then
  ...
  # Escape for sed
  export SOLRDATAPATH="${SOLRDATAPATH//\//\\/}"
  sed "s/@@ALFRESCO_SOLR_DIR@@/$SOLRDATAPATH/g" $ALF_HOME/solr/solr-tomcat-context.xml > /tmp/alfrescoinstall/solr.xml
